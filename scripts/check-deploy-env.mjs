...
const warn = (m) => { avisos++; console.log(`  \x1b[33m!\x1b[0m ${m}`); };
const fail = (m) => { fallos++; console.log(`  \x1b[31m✗\x1b[0m ${m}`); };
const seccion = (t) => console.log(`\n\x1b[1m${t}\x1b[0m`);

// ── Node ─────────────────────────────────────────────────────────────────
seccion('Node');
const major = Number(process.versions.node.split('.')[0]);
console.log(`  versión: ${process.version} (${process.arch})`);
if (major >= 20) ok('cumple el mínimo (>=20)');
else fail(`se requiere Node 20 o superior; better-sqlite3 y sharp traen binarios para esas versiones`);
...
else warn('package.json no declara `engines.node`: el selector de Node de cPanel puede elegir otra versión');
...
for (const mod of ['better-sqlite3', 'sharp']) {
  try {
    ...
    console.log(`     → suele resolverse con: npm rebuild ${mod}  (o npm ci con la versión de Node correcta)`);
  }
}
...
function tamanio(dir) {
  try {
    return execSync(`du -sk "${path.join(root, dir)}" 2>/dev/null | cut -f1`).toString().trim();
  } catch { return null; }
}
let totalKb = 0;
...
if (!fs.existsSync(envPath)) {
  ...
  if (env.CMS_ALLOW_INSECURE_COOKIE === '1') fail('CMS_ALLOW_INSECURE_COOKIE=1 no debe usarse en producción');
  ...
  if (upload.startsWith(path.join(root, 'public'))) fail('CMS_UPLOAD_DIR cae dentro de public/: Astro lo copiaría a dist/ en cada build');
  else ok(`CMS_UPLOAD_DIR fuera de public/ (${env.CMS_UPLOAD_DIR ?? 'por defecto'})`);

  if ((env.CMS_PUBLISH_CHECK_COMMAND ?? '').includes('build')) ok('CMS_PUBLISH_CHECK_COMMAND reconstruye el sitio');
  else fail('CMS_PUBLISH_CHECK_COMMAND no incluye un build: publicar no actualizaría el sitio servido');

  if (env.PUBLIC_ENABLE_CMS === '1') ok('PUBLIC_ENABLE_CMS=1 (necesario para administrar desde el servidor)');
  else warn('PUBLIC_ENABLE_CMS distinto de 1: el sitio no tendrá interfaz de administración');

  if ((env.PUBLIC_CONTACT_EMAIL ?? '').includes('@hidromont.cl')) ok(`formulario dirigido a ${env.PUBLIC_CONTACT_EMAIL}`);
  else fail(`PUBLIC_CONTACT_EMAIL = "${env.PUBLIC_CONTACT_EMAIL ?? '(sin definir)'}" — debe ser un buzón corporativo`);

  if ((env.CMS_ALLOWED_ORIGINS ?? '').includes('https://')) ok('CMS_ALLOWED_ORIGINS con orígenes HTTPS');
...
console.log(`  CPUs: ${os.cpus().length} · memoria total: ${(os.totalmem() / 1024 ** 3).toFixed(1)} GB`);
if (os.totalmem() / 1024 ** 3 < 1) warn('menos de 1 GB de memoria: `astro check` puede quedarse sin memoria');
